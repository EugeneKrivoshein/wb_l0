...
func (c *Cache) getCacheFromDatabase() {
	...
	if err != nil {
		log.Printf("%s: can't download from db or cache is empty: %v\n", c.name, err)
		return
	}
	...
	c.pos = pos
	c.mutex.Unlock()
	log.Printf("%s: cache downloaded from db: queue is: %v, next position in queue is: %v", c.name, c.queue, c.pos)
}
...
func (c *Cache) SetOrder(oid int64, o Order) {
	if c.bufSize > 0 {
		...
		log.Printf("%s: Order add to Cache, Order position in queue is %v\n", c.name, c.pos)
	} else {
...
func (c *Cache) GetOrderOutById(oid int64) (*OrderOut, error) {
	...
	if isExist {
		log.Printf("%s: order (id:%d) taken from cache!\n", c.name, oid)
	} else {
		o, err = c.DBInst.GetOrderByID(oid)
		if err != nil {
			log.Printf("%s: err getting order: %v\n", c.name, err)
			return ou, err
		}
		c.SetOrder(oid, o)
		log.Printf("%s: order (id:%d) taken from db and saved to cache!\n", c.name, oid)
	}
